...
def cal_delta(lidar2img, img_width=1920, img_height=1080, step=0.1, info=None, thre=[-3,3]):
    '''
...
def vis_one(cld_coords, img, lane):
    img_pil = PIL.Image.fromarray(img)
    point_tuple = [tuple(p) for p in cld_coords]
    for point in point_tuple:
            center = point
            radius = 2
            left_top = (center[0] - radius, center[1] - radius)
            right_bottom = (center[0] + radius, center[1] + radius)
            PIL.ImageDraw.Draw(img_pil).ellipse([left_top, right_bottom], fill=COLORS[1])
    if lane is not None:
        for i in range(len(lane)):
            lane_tuple = [tuple(p) for p in lane[i]]
            PIL.ImageDraw.Draw(img_pil).line(
                xy=lane_tuple, fill=COLORS[i + 1], width=7)
...
def deal_lane_cv2cld(
    ...
    with open(manNmodY, 'rb') as f:
        diff_data = pickle.load(f)
    
    for i in range(len(diff_data)):
        try:
            file_fn_path = diff_data[i]['image_name']
            frame_id = str(diff_data[i]['frame_id'])
            file_name = file_fn_path.split('/')[-1]
            bag_name = file_name.split('_video')[0]
            bag_paths = osp.dirname(osp.dirname(file_fn_path))
            cld_path = osp.join(bag_paths, 'model_infer', bag_name, 'lidar_lane_seg_v2', bag_name + '.pkl')
            with open(cld_path, 'rb') as f:
                cld_data = pickle.load(f)
            c = [x for x in range(len(cld_data)) if cld_data[x]['img_filenames'].tolist()[1].split('/')[-2] == frame_id][0]
            pandar_flu2imgs_fn = cld_data[c]['pandar_flu2imgs'][1]
            point_coords = cld_data[c]['point_coords']
            prediction_cls = cld_data[c]['prediction_cls_name']
            ...
            fn_img = cv2.imread(file_fn_path)
            ...
            if len(tmp_result) != 0:
                ...
                break
            result = []
            img_vis = vis_one(cld_points, fn_img, tmp_result)
            save_path = osp.join(output_dir,"vis_output" ,file_name)
            os.makedirs(osp.join(output_dir,"vis_output"), exist_ok=True)
            ...
            if len(result) > 1 and len(cld_points):
                res_dis, cost = com_dis(
                    ...
                    img_width=W, img_height=H, use_thr=True, thre=500, file_path=file_fn_path)
                ...
                if len(res_dis) > 1:
                    img_unfix = vis_one(cld_points, fn_img, tmp_result)
                    color = (0, 0, 255)  # 绿色
                    thickness = 1
                    res_info = file_name+ ',pitch_delta:'+str(cost[0])+ ', yaw_delta:'+str(cost[1]) + ', roll_delta:'+str(cost[2]) + '\n'
                    res_info1 = res_info.split(', ')[0]
                    res_info2 = res_info.split(',')[1:3]
                    res_info2 = ' '.join(res_info2)
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    font_scale = 1
                    cv2.putText(img_unfix, res_info1, (50,100), font, font_scale, color, thickness)
                    cv2.putText(img_unfix, res_info2, (50,150), font, font_scale, color, thickness)
                    # debug
                    save_path = osp.join(output_dir, "unfix" , file_name)
                    ...
                    cv2.imwrite(save_path, img_unfix)
                else:
                    pass    
            else:
                continue    
...
if __name__ == "__main__":
    model_path = '/mnt/qts/usr/yifei.fan/workspace/conditional-lane-detection/models/culane_small.pth'
    model_cfg = "/mnt/qts/usr/yifei.fan/workspace/conditional-lane-detection/configs/condlanenet/tusimple/tusimple_small_test.py"
    output_dir = '/mnt/qts/usr/yifei.fan/workspace/conditional-lane-detection/1008_debug'
    manNmodY = '/mnt/nas/usr/yifei.fan/workpath/0928_random500.pkl'
    depth_thr = 300
    predict_thre = 0.5
    deal_lane_cv2cld(model_path, model_cfg, output_dir, manNmodY,
